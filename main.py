...
async def auth_user(user_in: UserIn): # async es un hilo nuevo (thread en Java)
    user_in_db = get_user(user_in.username)
    if user_in_db == None:
        raise HTTPException(status_code=404, detail="El usuario no existe") # Arroja el código del error http
    if user_in_db.password != user_in.password:
        return {"Autenticado": False}
    return {"Autenticado": True}

# Funcionalidad get_balance (función de servicio web)
...
# Funcionalidad make_transaction (función de servicio web)
